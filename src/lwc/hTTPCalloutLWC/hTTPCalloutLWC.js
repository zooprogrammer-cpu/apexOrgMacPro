...
export default class HTTPCalloutLWC extends LightningElement {
    ...
    // Using client side to make http callout
    ...
    // Using server side apex controller to make callout:
    async handleCurrencyConversion() {
        // rest api call
        const queryValue = `function=CURRENCY_EXCHANGE_RATE&from_currency=${this.fromCurrencyValue}&to_currency=${this.toCurrencyValue}`;
        const API_KEY =  `ENNH2GLEOOFFK1V6`;
        const endPoint = `https://www.alphavantage.co/query?${queryValue}&apikey=${API_KEY}`;
        const currencyDataResult = await getCurrencyData({strEndPointURL : endPoint});
        console.log(currencyDataResult);
        const exchangeData = currencyDataResult['Realtime Currency Exchange Rate'];
